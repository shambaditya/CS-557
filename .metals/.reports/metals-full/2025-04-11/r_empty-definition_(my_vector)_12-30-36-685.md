...
empty definition using pc, found symbol in pc: local15
empty definition using semanticdb

found definition using fallback; symbol address_north
offset: 1189
...
class topUnit(val N: Int, val W: Int, val paral : Int = 1, val K : Int = 16) extends Module {
    ...
    for (i <- 0 until N) {
        ...
        if (i >= 1) {
            group(i).io.addres@@s_north <> group(i - 1).io.address_south
