error id: file://<WORKSPACE>/src/main/scala/PE.scala:57
  ...
  val io = IO(new Bundle {
    ...
    switch(state) {
      is(idle){
        ...
        when(io.query.valid && io.refs.valid){ // && io.address@@_i.valid
          counter := 0.U
          state := running
    ...
    */
    ...
    when(reg_ready_to_enq_north && !(running && (tmp > dist_in_topk || counter === num_cycle.U - 1.U))){
      fifo_addr.io.enq.valid := true.B
      fifo_dist.io.enq.valid := true.B
