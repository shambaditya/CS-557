...
//     size_t partitionSizeBytes = partitionSizeInts * sizeof(uint16_t);
...
//     int intsPerPartition = partitionSizeBytes / sizeof(uint16_t);
...
//         memcpy(writeLocation + rowOffsetInPartition * NUM_DIM, row_data, NUM_DIM * sizeof(uint16_t));
...
//     uint16_t* queryVector = (uint16_t*)malloc(NUM_DIM * sizeof(uint16_t));
//     if (queryVector == NULL) {
...
//     uint16_t *host_alloc = (uint16_t*) total_alloc.getHostAddr();
...
//     remote_ptr query_alloc = handle.malloc(NUM_DIM * sizeof(uint16_t));
//     uint16_t *fpga_query = (uint16_t*) query_alloc.getHostAddr();
    
//     // Copy the query vector from host memory to FPGA memory
//     memcpy(fpga_query, queryVector, NUM_DIM * sizeof(uint16_t));
    
//     // Transfer data to FPGA
//     dma_workaround_copy_to_fpga(total_alloc);
//     dma_workaround_copy_to_fpga(query_alloc);
//     Project::integration(0, total_alloc, query_alloc)
